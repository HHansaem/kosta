...
public class Exercise11_06 {
	...
	//	다음의 코드는 성적평균의 범위별로 학생 수를 세기 위한 것이다. TreeSet이 학생들의 평균을 기준으로 
	//	정렬하도록 compare(Object o1, Object o2)와 평균점수의 범위를 주면 해당 범위에 속한 학생의 수를 
	//	반환하는 getGroupCount()를 완성하라.
	//	 [Hint] TreeSet의 subSet(Object from, Object to)를 사용하라

	static int getGroupCount(TreeSet<Student1> tset, int from, int to) {
		Student1 fromStud = new Student1(null, 0, 0, from, from, from);
		Student1 toStud = new Student1(null, 0, 0, to, to, to);
		SortedSet<Student1> sset = tset.subSet(fromStud, toStud);
		return sset.size();
	}
	...
	public static void main(String[] args) {
		TreeSet<Student1> set = new TreeSet<>(new Comparator<Student1>() {
			...
			public int compare(Student1 o1, Student1 o2) {
				if(o1.getAverage() > o2.getAverage()) {  //값을 비교해서
					return 1;  //오름차순
